...
public class CircleProgressbar extends TextView {
    ...
    // 内部圆的颜色
    private ColorStateList inCircleColors = ColorStateList
    ...
    private void initialize(Context context, AttributeSet attributeSet) {
        ...
        if (typedArray.hasValue(R.styleable.CircleProgressbar_in_circle_color))
            inCircleColors = typedArray
                    .getColorStateList(R.styleable.CircleProgressbar_in_circle_color);
        else
            inCircleColors = ColorStateList.valueOf(Color.TRANSPARENT);
        circleColor = inCircleColors.getColorForState(getDrawableState(),
                Color.TRANSPARENT);
        progressLineColor = typedArray.getColor(R.styleable.CircleProgressbar_progress_color, Color.WHITE);
        typedArray.recycle();
    }
    ...
    private void validateCircleColor() {
        int circleColorTemp = inCircleColors.getColorForState(
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int size = bounds.height() > bounds.width() ? bounds.width() : bounds
                .height();
        float outerRadius = size / 2;

        // 画内部背景
        int circleColor = inCircleColors
...
//        canvas.drawCircle(bounds.centerX(), bounds.centerY(), outerRadius
//                - outLineWidth / 2, mPaint);
...
//        canvas.drawText(getText().toString(), bounds.centerX(), textY, paint);
        ...
        mArcRect.set(bounds.left + deleteWidth / 2, bounds.top + deleteWidth
                / 2, bounds.right - deleteWidth / 2, bounds.bottom
                - deleteWidth / 2);
        ...
        public void run() {
            ...
            if (progress >= 0 && progress <= 100) {
                if (mCountdownProgressListener != null)
                    mCountdownProgressListener.onProgress(progress);
                invalidate();
