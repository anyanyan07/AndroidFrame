...
public class StatusBarUtil {
    ...
    public static void setColorForVpFragment(FragmentActivity activity, @ColorInt int color, View view) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
        setLightMode(activity, color);
        if (view != null) {
            int statusHeight = getStatusBarHeight(activity);
            int actionBarHeight = (int) getActionBarHeigth(activity);
            ViewGroup.LayoutParams ll = view.getLayoutParams();
    ...
    public static void setLightMode(Activity activity, @ColorInt int color) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
